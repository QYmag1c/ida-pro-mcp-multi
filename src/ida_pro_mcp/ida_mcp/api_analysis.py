...
def decompile(
    ...
    max_lines: Annotated[int, "Max lines to return (default: 500, max: 2000, 0 for unlimited)"] = 500,
    offset: Annotated[int, "Line offset to start from (default: 0)"] = 0,
) -> dict:
    """Decompile function to pseudocode with pagination support for large functions
    
    For large functions, use offset and max_lines to paginate through the code.
    The response includes a cursor field indicating if there are more lines.
    
    Example usage for large functions:
    1. First call: decompile("0x401000") - returns first 500 lines
    2. If cursor.done is false: decompile("0x401000", offset=500) - returns next 500 lines
    3. Continue until cursor.done is true
    """
    # Enforce limits
    if max_lines < 0:
        max_lines = 0
    ...
    try:
        ...
        # If unlimited or code is small enough, return everything
        lines = code.split('\n')
        ...
        if offset >= total_lines:
            return {
                ...
                "offset": offset,
                "cursor": {"done": True},
                "error": f"Offset {offset} exceeds total lines {total_lines}"
            }
        ...
        return {
            ...
            "offset": offset,
            "cursor": {"next": end_line} if has_more else {"done": True}
        }
...
def decompile_all(
    addr: Annotated[str, "Function address to decompile"],
    chunk_size: Annotated[int, "Lines per chunk for response (default: 300)"] = 300,
) -> dict:
    """Decompile function and return ALL code in multiple chunks within a single response
    
    Unlike decompile() which paginates, this returns the complete decompilation
    split into manageable chunks. Useful when you need the full code at once.
    
    For very large functions (>5000 lines), consider using decompile() with pagination instead.
    """
    if chunk_size < 50:
        chunk_size = 50
